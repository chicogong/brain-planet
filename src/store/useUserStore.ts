...
interface UserState {
  ...
  adventureProgress: number; // 0 = first level, 1 = second, etc.
  ...
  advanceAdventure: (levelIndex: number) => void;
}
...
export const useUserStore = create<UserState>()(
  persist(
    (set, get) => ({
      ...
      claimDailyQuest: () =>
        set((state) => {
          ...
          if (
            ...
            state.dailyQuest.playCount >= 3 &&
            !state.dailyQuest.claimed
          ) {
      ...
      advanceAdventure: (levelIndex) =>
        set((state) => {
          if (levelIndex >= state.adventureProgress) {
            return { adventureProgress: levelIndex + 1 };
          }
          return state;
        }),
    }),
