      ...
      <div className="w-full bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-gray-100 min-h-[400px] flex flex-col items-center relative overflow-hidden">
          ...
          <div className="w-full h-full flex flex-col">
            ...
            {(score !== undefined || timeLeft !== undefined) && (
              <div className="flex justify-between w-full mb-8 px-4">
                {timeLeft !== undefined && (
                  <div
                    className={`px-4 py-2 rounded-2xl font-black text-xl ${bgSoftClass} ${textClass}`}
                  >
                    ⏳ {timeLeft} s
                  </div>
                )}
            ...
            <div className="flex-1 w-full">{playingContent}</div>
          </div>
        )}
        ...
        {gameState === "won" && (
          ...
          >
            ...
            {wonContent && <div className="w-full max-w-sm mb-8">{wonContent}</div>}
              ...
              <button
                onClick={async () => {
                  ...
                  if (navigator.share && /mobile|android|iphone|ipad/i.test(navigator.userAgent)) {
                    try {
                      await navigator.share({ title: "脑力星球 - 战绩分享", text: shareText, url });
                    } catch (e) {}
                  } else {
