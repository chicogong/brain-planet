          ...
          <form onSubmit={handleVerify} className="space-y-6">
            ...
            <input
              ...
              className={`w-full text-center text-3xl font-bold p-4 border-2 rounded-2xl focus:outline-none transition-colors ${
                error ? "border-red-500 bg-red-50 text-red-500" : "border-gray-200 focus:border-indigo-500"
              }`}
              placeholder="输入答案"
              autoFocus
            />
  ...
  return (
    <div className="flex flex-col items-center max-w-4xl mx-auto w-full px-4 pb-12">
      <div className="w-full flex items-center mb-8 sticky top-0 bg-gray-50/80 backdrop-blur-md py-4 z-10">
        <Link href="/" className="p-2 bg-white rounded-full shadow-sm hover:bg-gray-50 transition-colors mr-4">
          <ArrowLeft className="w-6 h-6 text-gray-600" />
        </Link>
        <div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-3">
            {AVAILABLE_BADGES.map((badge) => {
              ...
              return (
                <div key={badge.id} className={`flex items-center gap-3 p-3 rounded-2xl border ${isUnlocked ? 'border-gray-100 bg-gray-50' : 'border-transparent opacity-50 grayscale'}`}>
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${isUnlocked ? badge.color : 'bg-gray-200'}`}>
                    {badge.icon}
                  </div>
                  <div>
                  ...
                  {isUnlocked && <div className="ml-auto text-green-500 text-sm font-bold">已解锁</div>}
                </div>
              );
            })}
          </div>
